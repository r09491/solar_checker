...
def _get_kwh_bar_unified(
    ...
    if smeonon.any():
        ...
        if smeoffon.any():
            bonus = balconyon.sum()-smeoffon.sum()
            title += f' | Bonus {bonus:.1f}kWh'
            title += f'~{(bonus*price):.2f}€'
            title += f' ({bonus/balconyon.sum()*100:.0f}%)'
        ...
        title += f' ({smeonon.sum()/(smeonon.sum()+bonus)*100:.0f}%)'
