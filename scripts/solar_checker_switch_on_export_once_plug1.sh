...
tail -n 10 $SOLAR_CHECKER_LATEST|solar_checker_switch_on_export_once.py \
     --plug_name plug1 \
     --power_mean_import_open 20 \
     --power_mean_export_closed 20 \
     --power_mean_deviation 15 \
     --power_samples 10 2>> $SOLAR_CHECKER_ERROR

# This script shall start/stop the charging of low power devices. It
# starts the charging if power is exported after a certain period (10
# minutes).  It stops the charging if no power is exported to the
# provider after a certain period (10 minutes). At the end it shall
# ensure that the export is minmized.
